...
let ranks = ["normal1", "normal2", "altin", "elmas", "hazir", "topluluk", "api"];
for (let rank in ranks) {
  db[ranks[rank]] = new bookman(ranks[rank]);
}

const IDler = {
  botID: "727194000305422507",
  botToken: "NzI3MTk0MDAwMzA1NDIyNTA3.XvoSMA.VXVcSuoKccdJCWrIMV5lakhOMT8",
  botSecret: "YaY3iuU2FoULMrfLPeho0oQ8vc2Dlp_f",
  botCallbackURL: "https://code-universe.glitch.me/callback",
  sunucuID: "779133571373072384",
  sunucuDavet: "https://discord.gg/4sypp9S3J4",
  kodLogKanalı: "779268007611727883",
  sahipRolü: "779142714834681856",
  adminRolü: "779137126511673374",
  kodPaylaşımcıRolü: "779143679243583548",
  boosterRolü: "779362073699418113",
  kodPaylaşamayacakRoller: ["779145595587002428"],
  hazırAltyapılarRolü: "779154242241167390",
  elmasKodlarRolü: "779153051276541962",
  altinKodlarRolü: "779154244682121226 ",
  normal1KodlarRolü: "779155821769916456",
  normal2KodlarRolü: "779156021489827890"
};
...
app.get("/normal1", (req, res) => {
  var data = db.normal1.get("kodlar");
  data = sortData(data);
...
app.get("/normal1/:id", (req, res) => {
  ...
  if (!id) req.redirect("/");
  let data = db.normal1.get("kodlar"); var code = findCodeToId(data, id);
  if (code) {
    res.render("kod", {
      user: req.user,
      kod: code
...
app.get("/normal2", (req, res) => {
  var data = db.normal2.get("kodlar");
  data = sortData(data);
...
app.get("/normal2/:id", (req, res) => {
  ...
  var id = req.params.id;
  if (!id) req.redirect("/");
  let data = db.normal2.get("kodlar");
...
app.get("/yol1", (req, res) => {
  res.render("yol1", {
    user: req.user
  });
});

app.get("/yol2", (req, res) => {
  res.render("yol2", {
    user: req.user
  });
});
...
app.post("/paylasim", (req, res) => {
  ...
  client.channels.cache.get(IDler.kodLogKanalı).send(
    ...
    .addField("<a:Flight:771733258907025409> Koda gitmek için tıkla!", `[Tıkla!](https://code-universe.glitch.me/${obj.kod_rank}/${obj.id})`));
